...
using Solarius;
using Solarius.Program;
using Solarius.Errors;
using Solarius.Accounts;
...
namespace Solarius
{
    ...
    namespace Errors
    {
        public enum SolariusErrorKind : uint
        {
    ...
    public partial class SolariusClient : TransactionalBaseClient<SolariusErrorKind>
    {
        public SolariusClient(IRpcClient rpcClient, IStreamingRpcClient streamingRpcClient, PublicKey programId) : base(rpcClient, streamingRpcClient, programId)
        {
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.SolariusProgram.InitPlayer(accounts, levelSeed, programId);
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.SolariusProgram.ChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.SolariusProgram.SuperChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }

        protected override Dictionary<uint, ProgramError<SolariusErrorKind>> BuildErrorsDictionary()
        {
            return new Dictionary<uint, ProgramError<SolariusErrorKind>>{{6000U, new ProgramError<SolariusErrorKind>(SolariusErrorKind.NotEnoughEnergy, "Not enough energy")}, {6001U, new ProgramError<SolariusErrorKind>(SolariusErrorKind.WrongAuthority, "Wrong Authority")}, };
        }
    }
    ...
    namespace Program
    {
        ...
        public static class SolariusProgram
        {
